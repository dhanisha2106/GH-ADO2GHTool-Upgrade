...
public class GithubApi
{
    ...
    public virtual async Task<bool> GetBranch(string org, string repo, string branch)
    {
        var url = $"{_apiUrl}/repos/{org.EscapeDataString()}/{repo.EscapeDataString()}/branches/{branch.EscapeDataString()}";
        ...
        {
            throw new OctoshiftCliException($"Branch '{branch}' not found in repository {org}/{repo}");
        }
    }
    ...
    public virtual async Task CreateBranchProtection(string org, string repo, string branch, GithubBranchProtection protection)
    {
        ...
        {
            // Check if this is a GitHub plan limitation
            if (ex.Message.Contains("Upgrade to GitHub Pro", StringComparison.OrdinalIgnoreCase) ||
                ex.Message.Contains("make this repository public", StringComparison.OrdinalIgnoreCase))
            {
                throw new OctoshiftCliException($"Branch protection requires GitHub Pro, Team, or Enterprise for private repositories. Either make the repository public or upgrade your GitHub plan.", ex);
            }
            throw new OctoshiftCliException($"Permission denied when setting branch protection. Ensure your PAT has 'repo' scope and admin access to the repository.", ex);
        }
        catch (HttpRequestException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
        {
            throw new OctoshiftCliException($"Repository or branch '{branch}' not found. Ensure the branch exists in GitHub before applying protection.", ex);
        }
        catch (HttpRequestException ex) when (ex.StatusCode == System.Net.HttpStatusCode.UnprocessableEntity)
        {
            throw new OctoshiftCliException($"Invalid branch protection configuration. Check the policy settings and try again.", ex);
        }
    }

    public virtual async Task<GithubBranchProtection> GetBranchProtection(string org, string repo, string branch)
    {
        var url = $"{_apiUrl}/repos/{org.EscapeDataString()}/{repo.EscapeDataString()}/branches/{branch.EscapeDataString()}/protection";
        ...
        try
        {
            ...
            {
                RequiredStatusChecks = data["required_status_checks"] != null
                    ? new RequiredStatusChecks
                    {
                        Strict = (bool)data["required_status_checks"]["strict"],
                        Contexts = data["required_status_checks"]["contexts"]?.ToObject<List<string>>() ?? []
                    }
                    : null,
                EnforceAdmins = (bool?)data["enforce_admins"]?["enabled"],
                RequiredPullRequestReviews = data["required_pull_request_reviews"] != null
                    ? new RequiredPullRequestReviews
                    {
                        DismissStaleReviews = (bool)data["required_pull_request_reviews"]["dismiss_stale_reviews"],
                        RequireCodeOwnerReviews = (bool)data["required_pull_request_reviews"]["require_code_owner_reviews"],
                        RequiredApprovingReviewCount = (int)data["required_pull_request_reviews"]["required_approving_review_count"],
                        RequireLastPushApproval = (bool?)data["required_pull_request_reviews"]["require_last_push_approval"] ?? false
                    }
                    : null,
                RequiredLinearHistory = (bool?)data["required_linear_history"]?["enabled"] ?? false,
                AllowForcePushes = (bool?)data["allow_force_pushes"]?["enabled"] ?? false,
                AllowDeletions = (bool?)data["allow_deletions"]?["enabled"] ?? false,
                RequiredConversationResolution = (bool?)data["required_conversation_resolution"]?["enabled"] ?? false
            };
        }
        catch (HttpRequestException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
        {
            // No protection rules exist for this branch
            return null;
        }
        catch (HttpRequestException ex) when (ex.StatusCode == System.Net.HttpStatusCode.Forbidden)
        {
            // Forbidden - might be plan limitation or insufficient permissions
            // Return null to indicate no existing protection (don't fail the check)
            return null;
        }
    }

    public virtual async Task<int> CreatePullRequest(string org, string repo, string title, string body, string head, string baseRef)
    {
        var url = $"{_apiUrl}/repos/{org.EscapeDataString()}/{repo.EscapeDataString()}/pulls";
        ...
        {
            throw new OctoshiftCliException($"Unable to create pull request. The branches may not exist or may already be merged. Head: {head}, Base: {baseRef}", ex);
        }
        catch (HttpRequestException ex) when (ex.StatusCode == System.Net.HttpStatusCode.Forbidden)
        {
            throw new OctoshiftCliException($"Permission denied when creating pull request. Ensure your PAT has 'repo' scope.", ex);
        }
    }

    public virtual async Task ClosePullRequest(string org, string repo, int prNumber)
    {
        var url = $"{_apiUrl}/repos/{org.EscapeDataString()}/{repo.EscapeDataString()}/pulls/{prNumber}";
    ...
    public virtual async Task CreatePullRequestComment(string org, string repo, int prNumber, string body)
    {
        var url = $"{_apiUrl}/repos/{org.EscapeDataString()}/{repo.EscapeDataString()}/issues/{prNumber}/comments";
    ...
    public virtual async Task AddLabelsToIssue(string org, string repo, int issueNumber, string[] labels)
    {
        ...
        var url = $"{_apiUrl}/repos/{org.EscapeDataString()}/{repo.EscapeDataString()}/issues/{issueNumber}/labels";
